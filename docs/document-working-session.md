...
| **开个话题决定** | 建新话题，**两份提案 + 相关批注**作为 refs 喂进去 |

> **修订（实现时拍板）**：原文写的是「两份提案 + 两个**来源话题**作为 refs」，
> 已改为不引用来源话题。理由：把一个话题的完整流水灌进另一个话题，等于把**过程**
> 重新混进本该只收**结论**的地方，与本系统的分层原则冲突；且流水冗长、多为噪音。
> 「为什么这么改」由**批注**承载 —— 那正是结论所在。
> 提案的来源话题以**标题**形式作为属性出现（标签，不是可展开的引用）。
>
> 代价：争用发生时来源话题可能尚未写回结论，此时新话题拿不到推理过程。
> 判断是可接受 —— 提案文本本身已体现意图，不够时人补一句即可，反正裁决本来就是人在做。

**没有"手动融合"**。见开头的不变量：想要第三版就去话题里说。

第二个出口是这套设计里最顺的一环 —— 争用本身就是个值得讨论的议题，
而讨论在本系统里已经是一等公民。喂进去的上下文是：两版文本、各自出自哪个话题（标题）、
以及那些话题在解决哪些批注。

**新建空话题，而不是从某一方 fork。** 从 A fork 会让新话题继承 A 的全部推理和立场，
B 只能当外来材料塞进去 —— 裁决从一开始就偏向 A。裁决要的是中立。
